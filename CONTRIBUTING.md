...
```bash
gh pr list --repo adammarquette/MarqSpec.Client.Finnhub --state open
git ls-remote --heads https://github.com/adammarquette/MarqSpec.Client.Finnhub
```
...
There is no in-process fake gateway in this repo. The unit suite stubs the HTTP/websocket transport; the
integration suite talks to a loopback listener and needs **no credentials**. Live-credentialed tests are opt-in
and separately tagged; see the [QA contract](MarqSpec.Client.Finnhub.IntegrationTests/AGENTS.md).
